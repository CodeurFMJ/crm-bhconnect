...
<head>
    ...
    <title>Nouvel Objectif de Performance - CRM-Bh Connect</title>
            ...
            <main class="col-12 col-md-9 col-lg-10">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div class="page-header">
                        <h1 class="h3 mb-0">Nouvel Objectif de Performance</h1>
                        <p class="text-muted">Définir un nouvel objectif de performance</p>
                    </div>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.performance-objectives.store') }}">
                            @csrf
                            
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label class="form-label">Nom de l'objectif <span class="text-danger">*</span></label>
                                    <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="Ex: Chiffre d'affaires mensuel" />
                                    @error('name')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                                        ...
                                        <option value="revenue" {{ old('type') === 'revenue' ? 'selected' : '' }}>Chiffre d'affaires</option>
                                        <option value="clients" {{ old('type') === 'clients' ? 'selected' : '' }}>Nombre de clients</option>
                                        <option value="appointments" {{ old('type') === 'appointments' ? 'selected' : '' }}>Rendez-vous</option>
                                        <option value="tasks" {{ old('type') === 'tasks' ? 'selected' : '' }}>Tâches</option>
                                        <option value="conversion" {{ old('type') === 'conversion' ? 'selected' : '' }}>Taux de conversion</option>
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="row mb-3">
                                <div class="col-md-4">
                                    ...
                                    <input name="target_value" type="number" step="0.01" min="0" class="form-control @error('target_value') is-invalid @enderror" value="{{ old('target_value') }}" required placeholder="0.00" />
                                    @error('target_value')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <input name="unit" type="text" class="form-control @error('unit') is-invalid @enderror" value="{{ old('unit', 'FCFA') }}" required placeholder="FCFA, nombre, %" />
                                    @error('unit')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" placeholder="Description détaillée de l'objectif...">{{ old('description') }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                <div class="form-check">
                                    <input name="is_active" type="checkbox" class="form-check-input" id="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }}>
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-save me-1"></i>Créer l'objectif
                                </button>
